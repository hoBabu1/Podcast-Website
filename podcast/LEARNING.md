...
## Going live: switching from testnet to mainnet (chain ID 8453)

Up to now the whole payment flow ran on **Base Sepolia**, a *testnet*. A testnet is a full copy of the blockchain that behaves exactly like the real thing — same code, same wallets, same transactions — except the coins are worthless play money you mint for free. It exists so you can break things safely: a bug that "loses $50" on testnet loses nothing real. **Mainnet** is the production network where tokens are actual money. Flipping from one to the other is mostly a config change, but it's the highest-stakes config change in the app, because from this point a mistake moves real funds.

**What is a "chain ID"?** Every EVM network has a unique number that identifies it. Base mainnet is `8453`; Base Sepolia is `84532`. Wallets and libraries use this number to make sure everyone is talking about the same network — if your app says "I'm on 8453" but the user's wallet is connected to 84532, the wallet will refuse or prompt to switch, because a transaction signed for one chain is meaningless on another. That's why the switch touches `NEXT_PUBLIC_CHAIN_ID`, the wagmi config (`chains: [base]`), and the server verifier (`chain: base`) all at once: **client and server must agree on the chain**, or the user pays on one network while the server looks for the payment on another and never finds it.

**Why does the USDC address change?** A token like USDC isn't a built-in feature of the blockchain — it's a smart contract deployed at a specific address. Circle deployed USDC separately on each network, so the *same* token has a *different* address on each one:
- Base Sepolia USDC: `0x036CbD53842c5426634e7929541eC2318f3dCF7e`
- Base **mainnet** USDC: `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`

If you point the app at the testnet address while running on mainnet, transfers either fail or hit some unrelated/empty address. We verified the mainnet addresses by querying the live chain (`symbol`, `decimals`, and `eth_getCode`) rather than trusting a copy-paste — a good habit, because a wrong token address is an irreversible way to lose money. (This is also how we caught that a proposed USDT address had *no contract at all* on mainnet.)

**Decimals matter here too.** Real USDC and USDT on Base both use **6 decimals**, while our testnet mock token used 18. "Decimals" is just how many base units make one whole token; the contract only ever moves integers. `50 × 10^6` base units = 50 real USDC, but `50 × 10^18` units against a 6-decimal token would be 50 *trillion*. Client and server must use the same decimals *and* that number must match the token's real on-chain value — so moving to mainnet's 6-decimal tokens means 6 everywhere.
